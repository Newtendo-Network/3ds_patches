...
#include "patches/friends.h"
#include "patches/ssl.h"
#include "patches/act.h"
...
{
    if(areServiceNamesEqual(name, "frd:a", nameSize))
        Patch_Friends_Module(sessionData->pid);

    if(areServiceNamesEqual(name, "ssl:C", nameSize))
        Patch_SSL_Module(sessionData->pid);

    if(areServiceNamesEqual(name, "act:a", nameSize))
        Patch_ACT_Module(sessionData->pid);
